...
A deliberately separate web-enabled chat. It receives **no** Analyzer or Assistant context. Local by default: a planning step (local model, JSON) decides whether this turn needs the public web. Search only when the user asked to look something up or the task needs current/external facts. Simple “what is this?” on a photo stays local. Queries are short and never include OCR dumps, screenshots, logs or Analyzer JSON. The sources footer appears only when search actually ran. **Clear conversation** empties this transcript and this tab’s OCR cache; it does not touch Assistant.
...
- Log timestamps are treated as record boundaries when the shape is recognizable; numeric dates such as `09/01/26` do not get a calendar `time_range` unless the same log makes day/month order unambiguous. RFC3164 stamps are shown as written (`Dec 24 06:55:46`); a year is not filled in when the source has none. Oracle-style stamps that already include a year are parsed as calendar times. ISO stamps with `Z` or an offset keep that timezone on the reported range so SAML and log windows can be compared for overlap. A bracketed level after a timestamp (`[error]`, `[warn]`, …) is a source marker; Apache `notice` is counted as INFO. `crit` / `alert` / `emerg` are not mapped. Vendor codes are identifiers, not correlated incidents; occurrence count is not importance. `error:` in the text of an `ORA-` / `RMAN-` line is not a source ERROR.
