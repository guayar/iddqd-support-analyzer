...
- protocol/profile validation with structured error codes, including assertion/bearer/session/metadata time windows with configurable clock skew (`SAML_CLOCK_SKEW_SECONDS`, default 120)
...
- SAML time checks use analyzer UTC plus `SAML_CLOCK_SKEW_SECONDS` (default 120; `0` is strict). That covers small IdP/SP clock drift, not traces that expired hours or years ago.
...
`SAML_CLOCK_SKEW_SECONDS` (default 120, `0` = no extra tolerance) widens Conditions / bearer `NotOnOrAfter` / `NotBefore`, `SessionNotOnOrAfter`, and metadata `validUntil` the way a typical SP clock-skew window does: valid if `NotBefore − S ≤ now < NotOnOrAfter + S`.
