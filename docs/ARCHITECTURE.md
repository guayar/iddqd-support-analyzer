...
Gradio UI (app.py)
  ...
  │         └── Analyze result only after explicit Attach latest analysis; no websearch
  │
...
- Assistant (optional) does not invoke web search. It receives an Analyze result only after an explicit Attach latest analysis handoff.
