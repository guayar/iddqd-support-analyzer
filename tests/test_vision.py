...
from vision import VisionError, attach_images, clear_vision_cache, inspect_image
...
def test_accepts_png_and_caps_count(tmp_path):
    ...
    with patch("vision.ocr_image", side_effect=lambda p: {**inspect_image(p), "text": "ERROR 500", "error": None}):
...
def test_ocr_failure_still_attaches(tmp_path):
    ...
    with patch("vision.ocr_image", side_effect=lambda p: {**inspect_image(p), "text": "", "error": "Tesseract is not installed locally."}):
        attachments, notes = attach_images([path])
    assert len(attachments) == 1
    assert attachments[0]["ocr_error"]


def test_assistant_sends_image_and_ocr_and_keeps_web_chat_text_only(tmp_path):
    clear_vision_cache()
    ...
    with patch("chats.complete", fake_complete), patch(
        "vision.ocr_image",
        side_effect=lambda p: {**inspect_image(p), "text": "INVALID_ISSUER", "error": None},
    ...
    with patch("chats.complete", fake_web_complete), patch("websearch.web_search", return_value=([], ["q"], "auto")):
        web_chat("public question", [{"role": "user", "content": [{"path": path}, "secret screenshot"]}])
    sent = web_captured[0]
    assert all("images" not in m or not m.get("images") for m in sent)
    assert all("INVALID_ISSUER" not in str(m.get("content")) for m in sent)
    assert all("LOCAL OCR" not in str(m.get("content")) for m in sent)
...
def test_follow_up_resends_previous_screenshot(tmp_path):
    ...
    with patch("chats.complete", fake_complete), patch(
        "vision.ocr_image",
        side_effect=lambda p: {**inspect_image(p), "text": "ORA-12514", "error": None},
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        test_assistant_sends_image_and_ocr_and_keeps_web_chat_text_only(td)
